...
public class QnAListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(pageNo == 1 ){
			...
			pageBar += "<a href='"+request.getContextPath()+"/board/qnaList?cPage="+(pageNo-1)+"'>[이전]</a> ";
		}
		...
		while(!(pageNo>pageEnd || pageNo > totalPage)){
			...
			if(cPage == pageNo ){
				...
				pageBar += "<a href='"+request.getContextPath()+"/board/qnaList?cPage="+pageNo+"'>"+pageNo+"</a> ";
			}
			pageNo++;
		}
		...
		if(pageNo > totalPage){
			...
			pageBar += "<a href='"+request.getContextPath()+"/board/qnaList?cPage="+pageNo+"'>[다음]</a>";
		}
