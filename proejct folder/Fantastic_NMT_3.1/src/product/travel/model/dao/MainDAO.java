...
//		String query = "select * from nmt_air where air_code =(select air_code_depart from nmt_packagelist where package_code like ? and air_code_depart is not null)";
		String query = "select * from nmt_air where air_code =(select AIR_CODE_DEPART from NMT_PACKAGELIST where substr(PACKAGE_CODE,1,8) = ? and AIR_CODE_DEPART is not null)";
...
//		memberCode = "'%'"+memberCode+"'%'";
	...
	public Air selectPackageAirArrive(Connection conn, String memberCode) {
		...
		Air air = null;
//		String query = "select * from nmt_air where air_code =(select air_code_depart from nmt_packagelist where package_code like ? and air_code_depart is not null)";
		String query = "select * from nmt_air where air_code =(select AIR_CODE_ARRIVE from NMT_PACKAGELIST where substr(PACKAGE_CODE,1,8) = ? and AIR_CODE_ARRIVE is not null)";
		System.out.println(memberCode);
//		memberCode = "'%'"+memberCode+"'%'";
		try {
			pstmt = conn.prepareStatement(query);
